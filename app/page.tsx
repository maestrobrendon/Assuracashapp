...
import { WelcomePopupModal } from "@/components/modals/welcome-popup-modal"
...
export default function HomePage() {
  ...
  const checkWelcomePopup = async (userId: string) => {
    try {
      const { data: profile, error } = await supabase
        .from("profiles")
        .select("has_seen_welcome_popup")
        .eq("user_id", userId)
        .maybeSingle()

      if (error) {
        console.error("[v0] Error checking welcome popup status:", error)
        return
      }

      // Show popup if user hasn't seen it yet
      if (profile && !profile.has_seen_welcome_popup) {
        console.log("[v0] First-time user detected, showing welcome popup")
        setWelcomePopupOpen(true)
      }
    } catch (error) {
      console.error("[v0] Error checking welcome popup:", error)
    }
  }

  const handleActivateDemoAccount = async () => {
    try {
      const {
        data: { user },
      ...
      console.log("[v0] Activating demo account for user:", user.id)

      // Call the SQL function to create demo data
      ...
      console.log("[v0] Demo account activated successfully!")

      // Reload all data
      await Promise.all([loadWallets(), loadTransactions(), refetchMainWallet()])
  ...
  const checkAuth = async () => {
    ...
    if (!session) {
      ...
      if (user) {
        await checkWelcomePopup(user.id)
      }
    ...
    <div className="min-h-screen bg-background">
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <WelcomePopupModal
          ...
          onActivate={handleActivateDemoAccount}
        />
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card className="shadow-sm">
            ...
            <CardContent>
              <div className="space-y-4">
                {circles
                  .filter((circle) => circle.role === "admin")
                  .map((circle) => (
                    <div
                      key={circle.id}
