...
BEGIN
  ...
  -- Create demo main wallet with 500k balance and capture its ID
  ...
  IF demo_wallet_id IS NULL THEN
    SELECT id INTO demo_wallet_id 
    FROM public.main_wallets 
    WHERE user_id = new.id AND mode = 'demo' 
    LIMIT 1;
  END IF;
  ...
  -- Log the activity in transactions table with correct column names
  IF demo_wallet_id IS NOT NULL THEN
