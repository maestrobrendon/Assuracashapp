...
DROP FUNCTION IF EXISTS public.create_demo_data_for_user(UUID) CASCADE;

-- Create comprehensive demo data function with proper parameter
...
BEGIN
  ...
  RAISE NOTICE 'Created demo main wallet: %', v_demo_main_wallet_id;
  ...
  IF v_demo_main_wallet_id IS NOT NULL THEN
    ...
    RAISE NOTICE 'Created demo transaction: %', v_demo_transaction_id;
  END IF;
  ...
  RAISE NOTICE 'Created 3 budget wallets';
  ...
  RAISE NOTICE 'Created 3 goal wallets';
  ...
  RAISE NOTICE 'Created 5 circles and added user as admin';

  RAISE NOTICE 'Demo data creation completed successfully for user: %', p_user_id;
EXCEPTION
  WHEN OTHERS THEN
...
-- Create trigger function that fires on new user signup
...
BEGIN
  ...
  PERFORM public.create_demo_data_for_user(NEW.id);
...
GRANT EXECUTE ON FUNCTION public.create_demo_data_for_user(UUID) TO authenticated, anon;
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE '✅ Trigger on_auth_user_created is ACTIVE and will automatically create demo data for all new signups!';
  ELSE
    RAISE WARNING '❌ Trigger was not created successfully!';
  END IF;
END $$;
